...
export async function importTournamentOneWorkbook() {
  ...
  if ((characters || []).length < 72) {
    throw new Error(
      `This app is connected to Supabase project ${SUPABASE_REF} and can only see ${(characters || []).length} characters. ` +
      'Make sure you seeded the characters table in that same project, then refresh the app and try the import again.'
    )
  }
  ...
  if (existingTournament) {
    ...
    if (gameIds.length) {
      const deleteByGameIds = async (table) => {
...
export async function seedTournamentOneData() {
  return importTournamentOneWorkbook()
}
